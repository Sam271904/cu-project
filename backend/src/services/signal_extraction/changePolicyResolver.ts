import { ChangePolicySchema, type ChangePolicy } from '@e-cu/shared';
 ...
 */
export function parseChangePolicyEnvOverride(raw: string | undefined): ChangePolicy | null {
  if (raw === undefined || raw === null) return null;
  const v = String(raw).trim();
  if (!v) return null;
  const parsed = ChangePolicySchema.safeParse(v);
  return parsed.success ? parsed.data : null;
}

export function resolveChangePolicyFromSourceTypes(
  sourceTypes: readonly string[],
  override: ChangePolicy | null,
): ChangePolicy {
  if (override) return override;

  const set = new Set(sourceTypes.map((s) => String(s)));
  if (set.has('bookmark')) return 'USER_OVERRIDE';

  const onlyTech = set.size > 0 && [...set].every((t) => t === 'tech');
  if (onlyTech) return 'SOURCE_TRUSTED';

  if (set.has('social')) return 'EVIDENCE_WEIGHTED';
