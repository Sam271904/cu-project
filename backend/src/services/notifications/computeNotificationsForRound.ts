...
import { buildPushPayload, ReminderLevel, signalFingerprintFromSignalsJson } from './buildPushPayload';

const USER_ID = 'local-user';
const SIGNAL_SCHEMA_VERSION = 'v1-signals-0';

function scoreToReminderLevel(score: number): ReminderLevel | null {
...
export async function computeAndStoreNotificationsForRound(
  ...
  for (const cluster_id of clusterIds) {
    const dsRow = stmtLoad.get(cluster_id) as { signal_schema_version: string; signals_json: string };
    if (!dsRow) continue;
    ...
    const tl = db
      ...
      .get(roundId, cluster_id) as { cluster_kind: string } | undefined;
    if (!tl) continue;
    if (tl.cluster_kind !== 'event_update') continue;

    const signal = JSON.parse(dsRow.signals_json) as any;

    const signal_fingerprint = signalFingerprintFromSignalsJson(dsRow.signals_json, SIGNAL_SCHEMA_VERSION);
