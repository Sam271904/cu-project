...
- [x] Step 1: Pure functions — `reminderScoring.ts`（`computeEvidenceNovelty` Jaccard、`computeConclusionDeltaFromClaimHashes` v1 无 embedding、`computeConflictStrength` / `computeConflictDelta`、`computeSignificantChangeScore`）；`buildClaimTextFromDecisionSignals` 已存在于 `buildClaimText.ts`；**NotificationPolicy 权重/阈值** — `loadAppConfig()`：`PIH_REMINDER_W1|W2|W3`（归一化）、`PIH_REMINDER_HIGH_THRESHOLD` / `MEDIUM_THRESHOLD`；`computeNotificationsForRound` 使用配置；`/api/demo/status` & `/api/push/status` 返回 `notification_policy`
