...
- [x] Step 1: Pure functions — `reminderScoring.ts`（`computeEvidenceNovelty` Jaccard、`computeConclusionDeltaFromClaimHashes`、`computeConclusionDeltaFromEmbeddings`/`cosineSimilarity`（可选）、`computeConflictStrength` / `computeConflictDelta`、`computeSignificantChangeScore`）；`buildClaimTextFromDecisionSignals` 已存在于 `buildClaimText.ts`；**NotificationPolicy 权重/阈值** — `loadAppConfig()`：`PIH_REMINDER_*`、`PIH_CLAIM_EMBEDDING`+`PIH_EMBEDDING_MODEL`；`claim_embedding_json` 列；`clusterKindFromSignals`（含 `evidence_changed`）；首轮无 prev → `topic_drift`；`/api/demo/status` & `/api/push/status` 的 `notification_policy` 含 embedding 开关
