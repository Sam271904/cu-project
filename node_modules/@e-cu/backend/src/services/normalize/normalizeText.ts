...
function removeTemplateTail(input: string): string {
  ...
  let t = withoutEntities.replace(
    /\s*(?:\bread more\b|\bview original\b|\bcontinue reading\b|\bshow more\b)\s*$/i,
    ''
  );
  ...
  t = t.replace(
    /\s*[\[(（【{]?\s*(?:read more|view original|continue reading|show more)\s*[\])）】}]?\s*$/i,
    ''
  );
