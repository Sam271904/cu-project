...
const collectFromFeedUrl_1 = require("../adapters/rss/collectFromFeedUrl");
const normalizeRawItemsForRound_1 = require("../services/normalize/normalizeRawItemsForRound");
const clusterNormalizedItemsForRound_1 = require("../services/cluster/clusterNormalizedItemsForRound");
const extractSignalsForRound_1 = require("../services/signal_extraction/extractSignalsForRound");
const storeKnowledgeForRound_1 = require("../services/knowledge/storeKnowledgeForRound");
const computeNotificationsForRound_1 = require("../services/notifications/computeNotificationsForRound");
const dispatchQueuedNotifications_1 = require("../services/notifications/dispatchQueuedNotifications");
const buildPushPayload_1 = require("../services/notifications/buildPushPayload");
const pushFeature_1 = require("../pushFeature");
const loadStoredFeedsForCollect_1 = require("../services/feeds/loadStoredFeedsForCollect");
const bookmarkIds_1 = require("../services/bookmarks/bookmarkIds");
const loadProfile_1 = require("../services/personalization/loadProfile");
const scoreCluster_1 = require("../services/personalization/scoreCluster");
...
function defaultSourceIdFromFeedUrl(feedUrl) {
    return `rss_${sha256Hex(feedUrl)}`;
}
async function runIngestionPipeline(db, roundId, nowUtcIso, uiLang) {
    await (0, normalizeRawItemsForRound_1.normalizeRawItemsForRound)(db, roundId, nowUtcIso);
    await (0, clusterNormalizedItemsForRound_1.clusterNormalizedItemsForRound)(db, roundId, nowUtcIso);
    await (0, extractSignalsForRound_1.extractSignalsForRound)(db, roundId, uiLang);
    await (0, storeKnowledgeForRound_1.storeKnowledgeForRound)(db, roundId);
    if ((0, pushFeature_1.isPushPipelineEnabled)()) {
        return await (0, computeNotificationsForRound_1.computeAndStoreNotificationsForRound)(db, roundId, nowUtcIso);
    }
...
function createApiHandler(db) {
    return async function handleApiRequest(req, res) {
        ...
        if (url === '/api/feeds' && method === 'POST') {
            ...
            const source_type = body?.source_type === 'tech' ? 'tech' : body?.source_type === 'social' ? 'social' : null;
            if (!source_type) {
            ...
            const maxRow = db.prepare('SELECT COALESCE(MAX(sort_order), -1) AS m FROM rss_feed_configs').get();
        ...
        if (url === '/api/collect' && method === 'POST') {
            ...
            let socialRssFeeds = Array.isArray(body?.socialRssFeeds)
                ? body.socialRssFeeds
                : [];
            let techRssFeeds = Array.isArray(body?.techRssFeeds)
                ? body.techRssFeeds
                : [];
            if (useStoredFeeds) {
                const loaded = (0, loadStoredFeedsForCollect_1.loadStoredFeedsForCollect)(db);
            ...
            async function ingestRssFeeds(params) {
                ...
                for (const feed of params.feeds) {
                    ...
                    const items = await (0, collectFromFeedUrl_1.collectRssFromUrl)({
                    ...
                    const stmt = db.prepare(`INSERT INTO raw_items
            ...
            (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
                    for (const item of items) {
                        stmt.run(roundId, params.sourceType, item.source_id, sourceName, item.external_id, item.title, item.published_at ?? null, item.collected_at, item.url, item.excerpt_or_summary ?? null, item.author ?? null, item.language, item.timestamp_quality);
                        inserted += 1;
                    }
                }
                return inserted;
        ...
        `);
            ...
            for (const raw of items) {
                ...
                const normalized = (0, bookmarkIds_1.normalizeBookmarkUrlForId)(urlStr);
                if (!normalized)
                    continue;
                const extId = (0, bookmarkIds_1.bookmarkExternalIdFromUrl)(urlStr);
                if (!extId || seen.has(extId))
                    continue;
                seen.add(extId);
                ...
                const source_id = (0, bookmarkIds_1.bookmarkFolderSourceId)(folder);
                ...
                stmt.run(roundId, 'bookmark', source_id, source_name, extId, title, published_at, nowUtcIso, normalized, note || null, null, 'en', 'missing');
                inserted += 1;
            }
            if (inserted === 0) {
        ...
        VALUES
          (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
            async function runSeedRound(items) {
                const nowUtcIso = new Date().toISOString();
                const info = stmtRound.run('completed', 'v1-seed-test-data');
                const roundId = Number(info.lastInsertRowid);
                for (const it of items) {
                    stmtRaw.run(roundId, it.source_type, it.source_id, it.source_name, it.external_id, it.title, nowUtcIso, nowUtcIso, it.url, it.excerpt_or_summary, 'Seed Author', 'en', 'missing');
                }
                await (0, normalizeRawItemsForRound_1.normalizeRawItemsForRound)(db, roundId, nowUtcIso);
                await (0, clusterNormalizedItemsForRound_1.clusterNormalizedItemsForRound)(db, roundId, nowUtcIso);
                await (0, extractSignalsForRound_1.extractSignalsForRound)(db, roundId, uiLang);
          ...
          `)
                .all(targetRoundId);
            const kindSummary = kinds.reduce((acc, r) => {
        ...
        if (url === '/api/personalization' && method === 'GET') {
            ...
            const personaRows = db
                ...
                .all();
            const feedback = db
                .prepare(`SELECT cluster_id, sentiment, saved, updated_at_utc FROM personalization_feedback ORDER BY updated_at_utc DESC LIMIT 500`)
                .all();
            jsonResponse(res, 200, {
                success: true,
                keywords,
        ...
        if (url === '/api/personalization' && method === 'PUT') {
            ...
            const txn = db.transaction(() => {
                ...
                const insP = db.prepare('INSERT INTO personalization_personas (name, keywords_json, weight) VALUES (?, ?, ?)');
        ...
        if (url === '/api/personalization/feedback' && method === 'POST') {
            ...
            const nextSaved = body?.saved !== undefined ? (body.saved === true || body.saved === 1 ? 1 : 0) : (existing?.saved ?? 0);
        ...
        if (url === '/api/homepage' && method === 'GET') {
            const latest = db.prepare('SELECT id FROM collection_rounds ORDER BY id DESC LIMIT 1').get();
            if (!latest) {
                jsonResponse(res, 200, { decision_cards: [], topic_board: [], timeline_feed: [] });
                return;
            }
          ...
          `)
            ...
            const profile = (0, loadProfile_1.loadPersonalizationProfile)(db);
            const cardsRaw = rows.map((r) => {
                ...
                const changeSummary = parsedSignals?.change?.change_summary ?? '';
            ...
            const cardsScored = cardsRaw.map((c) => {
                const sc = (0, scoreCluster_1.scoreClusterForPersonalization)({
        ...
        if (url === '/api/knowledge/search' && method === 'GET') {
          ...
          `)
            ...
            if (tagFilters.length > 0) {
                parsedRows = parsedRows.filter((r) => tagFilters.some((tf) => r.tags.some((t) => {
        ...
        if (url === '/api/knowledge/cluster' && method === 'GET') {
          ...
          `)
            ...
            if (!row) {
                const rootId = (0, clusterNormalizedItemsForRound_1.resolveEvidenceRootClusterId)(db, clusterId);
                if (rootId !== clusterId)
                    row = loadKnowledge(rootId);
            }
            if (!row) {
            ...
            const evidenceLinks = Array.isArray(entryJson?.change?.evidence_links)
                ...
                : [];
            const timeline = evidenceLinks.map((l, idx) => ({
                id: `${clusterId}_${idx}`,
        ...
        if (url === '/api/demo/status' && method === 'GET') {
          ...
          `)
                .all(latestRoundId);
            const clusterKindSummary = kinds.reduce((acc, r) => {
                acc[r.cluster_kind] = r.cnt;
                return acc;
            }, {});
          ...
          `)
                .all(latestRoundId);
            function collectKeysDeep(x, out) {
                if (!x || typeof x !== 'object')
                    return;
                if (Array.isArray(x)) {
                    for (const i of x)
                        collectKeysDeep(i, out);
                    return;
                }
                const o = x;
            ...
            for (const row of signalsRows) {
                ...
                if (banned.some((b) => keys.has(b)))
                    hasForbiddenJsonKeys = true;
                ...
                for (const l of links) {
                    ...
                    if (snippet.length > maxSignalSnippetLength)
                        maxSignalSnippetLength = snippet.length;
                }
            }
            ...
            const privacyChecks = {
                pass: !hasForbiddenKnowledgeColumns &&
          ...
          `)
                .all();
            const notificationSummary = notifRows.reduce((acc, r) => {
                acc[r.status] = r.cnt;
                return acc;
            }, {});
        ...
        if ((url === '/api/push/status' ||
            ...
            url === '/api/push/send') &&
            !(0, pushFeature_1.isPushPipelineEnabled)()) {
        ...
        if (url === '/api/push/status' && method === 'GET') {
            const row = db.prepare('SELECT COUNT(*) AS cnt FROM notification_subscriptions').get();
        ...
        if (url === '/api/push/subscribe' && method === 'POST') {
            ...
            const pushPermissionStatus = typeof rawPerm === 'string' && allowedPerm.has(rawPerm) ? rawPerm : 'granted';
            const rawTs = body?.consent_timestamp;
            const consentTimestamp = typeof rawTs === 'string' && rawTs.trim() ? rawTs.trim() : new Date().toISOString();
            db.prepare(`
        ...
        if (url === '/api/push/send' && method === 'POST') {
            const body = (await readJsonBody(req));
            const mode = (body?.mode === 'real' ? 'real' : 'simulate');
            const limitRaw = body?.limit;
            const limit = typeof limitRaw === 'number' && Number.isFinite(limitRaw) ? Math.max(1, Math.floor(limitRaw)) : 50;
            const nowUtcIso = new Date().toISOString();
            try {
                const result = await (0, dispatchQueuedNotifications_1.dispatchQueuedNotifications)(db, {
                    userId: 'local-user',
                    mode,
                    limit,
                    nowUtcIso,
